...
class TestSuite(unittest.TestCase):
    ...
    def _test_file(self, basename: str) -> None:
        ...
        unindented = DjHTML(
            expected_output, extra_blocks={"weird_tag": "endweird"}, extra_middle_tags=["weird_middle"]
        ).indent(0)
        self.assertNotEqual(unindented, expected_output)

        # Re-indent the unindented output to 4
        actual_output = DjHTML(
            unindented, extra_blocks={"weird_tag": "endweird"}, extra_middle_tags=["weird_middle"]
        ).indent(4)
        self.assertEqual(expected_output, actual_output)

        # Compare the tokenization
        actual_tokens = DjHTML(
            actual_output, extra_blocks={"weird_tag": "endweird"}, extra_middle_tags=["weird_middle"]
        ).debug()
